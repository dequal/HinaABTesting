...
@interface HNABManager()

// 初始化配置
@property (nonatomic, strong) HinaABTestConfigOptions *configOptions;

@property (nonatomic, strong) HNABExperimentDataManager *dataManager;
...
@property (nonatomic, strong) HNABRequestManager *requestManager;

@end

@implementation HNABManager
...
- (instancetype)initWithConfigOptions:(nonnull HinaABTestConfigOptions *)configOptions {
    ...
    if (self) {
        ...
        NSString *serialQueueLabel = [NSString stringWithFormat:@"com.HinaABTest.HNABManager.serialQueue.%p", self];
        ...
        // 数据存储和解析
        _dataManager = [[HNABExperimentDataManager alloc] initWithSerialQueue:_serialQueue];

        _requestManager = [[HNABRequestManager alloc] init];
...
- (void)registerSAJSBridge:(NSNotification *)notification {
    ...
    if (![webView isKindOfClass:WKWebView.class]) {
        HNABLogError(@"notification object is invalid webView from SensorsAnalyticsSDK");
        return;
    }
...
- (void)messageFromH5:(NSNotification *)notification {
    ...
    if (![webView isKindOfClass:WKWebView.class]) {
        HNABLogError(@"Message webview is invalid from SensorsAnalyticsSDK");
        return;
    }

    NSDictionary *messageDic = [HNABJSONUtils JSONObjectWithString:message.body];
    if (![messageDic isKindOfClass:[NSDictionary class]]) {
        HNABLogError(@"Message body is formatted failure from JS SDK");
        return;
    }
    ...
    if (![dataDic isKindOfClass:NSDictionary.class]) {
        HNABLogError(@"Message data is invalid from JS SDK");
        return;
    }
    ...
    [HNABNetwork dataTaskWithRequest:requestData.request completionHandler:^(id _Nullable jsonObject, NSError *_Nullable error) {
        ...
        NSData *callJSData = [HNABJSONUtils JSONSerializeObject:callJSDataDic];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [webView evaluateJavaScript:javaScriptSource completionHandler:^(id _Nullable response, NSError *_Nullable error) {
                if (error) {
                    // 可能方法不存在；
                    HNABLogError(@"window.sensorsdata_app_call_js abtest error：%@", error);
                } else {
...
- (void)fetchABTestWithExperiment:(HinaABTestExperiment *)experiment {
    ...
    if (![HNABValidUtils isValidString:experiment.paramName]) {
        ...
        HNABLogError(@"paramName: %@ error，paramName must be a valid string!", experiment.paramName);
        return;
    }
...
/// 获取缓存试验
- (nullable id)fetchCacheABTestWithExperiment:(HinaABTestExperiment *)experiment {
    if (![HNABValidUtils isValidString:experiment.paramName]) {
        HNABLogError(@"paramName: %@ error，paramName must be a valid string!", experiment.paramName);
        return nil;
    }
...
/// 异步请求获取试验
- (void)fetchAsyncABTestWithExperiment:(HinaABTestExperiment *)experiment {
    NSError *error;
    // 验证自定义属性合法性，并统一修改自定义属性值为 String 类型
    NSDictionary *properties = [HNABPropertyValidator validateProperties:experiment.properties error:&error];
    if (error) {
        HNABLogError(@"%@", error.localizedDescription);
        dispatch_async(dispatch_get_main_queue(), ^{
            experiment.handler(experiment.defaultValue);
        });
        return;
    ...
    if (properties.count > 0 && experiment.paramName) {
        [requestData appendRequestBody:@{kHNABRequestBodyCustomProperties: properties, kHNABRequestBodyParamName: experiment.paramName}];
    }
    ...
    [self.dataManager asyncFetchAllExperimentWithRequest:requestData completionHandler:^(HNABFetchResultResponse *_Nullable responseData, NSError *_Nullable error) {
        __strong typeof(weakSelf) strongSelf = weakSelf;

        if (error || !responseData) {
            HNABLogError(@"asyncFetchAllExperimentWithRequest failure，error: %@", error);
            // 请求失败，主线程回调结果
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)trackABTestWithData:(SABExperimentResult *)resultData {
    ...
    [HNABBridge track:kSABTriggerEventName properties:properties];
}
...
- (void)setCustomIDs:(NSDictionary <NSString*, NSString*> *)customIDs {
    NSDictionary *tempCustomIDs = [HNABPropertyValidator validateCustomIDs:customIDs];
